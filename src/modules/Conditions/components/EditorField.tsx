...
export const EditorField: FC<EditorFieldProp> = ({
  rule,
  onDelete,
  onCaretChange,
  onElementChange,
      ...
      case "apn":
        return (
          <ComboBoxElement
            ...
            onDelete={onDelete}
            onChange={onElementChange}
            options={editorEntityEnv ? editorEntityEnv[element.type] : []}
          />
        );
  ...
  return (
    <div className="editor-panel">
      {rule.elements?.map((element, index) => (
        <span key={`stub_${index}`}>
          <CaretElement
            index={index}
            key={`carret_${index}`}
            isChecked={index === rule.cursorPosition}
      ...
      <CaretElement
        index={rule.elements.length}
        key={`carret_${rule.elements.length}`}
        isChecked={rule.elements.length === rule.cursorPosition}
        onCaretChange={onCaretChange}
      />
    </div>
  );
