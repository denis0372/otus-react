...
export class LoginForm extends Component<Props, State> {
  ...
  shouldComponentUpdate(nextState: State) {
    return (
      ...
      nextState.passw !== this.state.passw
    );
  }
    ...
    return (
      <div>
        <InputText
          ...
          onChangeAction={this.handleLoginChange}
        />
        <InputText
          placeHolder="Password"
          value={this.state.passw}
          onChangeAction={this.handlePasswChange}
        />
        <button onClick={this.loginAction}>Login</button>
        <div>
          Check:{" "}
          {this.state.login === this.successLogin &&
          this.state.passw === this.successPassw
            ? "ok"
            : "wrong"}
        </div>
      </div>
  ...
  loginAction() {
    alert(
      `Login: ${this.state.login}, passsw: ${this.state.passw}, result: ${
        this.state.passw === this.successPassw &&
        this.state.login === this.successLogin
      }`
    );
